...
```
    ...
    <tensorflow.python.keras.engine.functional.Functional object at 0x7f4e694433c8>
...
def compute_content_cost(content_output, generated_output):
    ...
    a_C_unrolled = tf.transpose(tf.reshape(a_C, shape=[m, n_H * n_W, n_C]), perm=[0,2,1])
    a_G_unrolled = tf.transpose(tf.reshape(a_G, shape=[m, n_H * n_W, n_C]), perm=[0,2,1])
...
```
    ...
    tf.Tensor(598.82837, shape=(), dtype=float32)
